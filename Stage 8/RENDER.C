...
void render_asteroids(UINT8 *base, Model *model)
{
    ...
    for (i = 0; i < MAX_ASTEROIDS; i += 1) {
        ...
        if (model->asteroid[i].delta_x == 2){
            clear_8_bitmap(base, model->asteroid[i].prev_x - (model->asteroid[i].delta_x * 2), model->asteroid[i].cur_y );
        }
...
void render_hearts(UINT16 *base, const Model *model)
{
    switch (model->lives) {
        case 3:
            plot_heart(base, 610, HEART_ROW);
            plot_heart(base, 600, HEART_ROW);
            plot_heart(base, 580, HEART_ROW);
            break;
        case 2:
            plot_heart(base, 610, HEART_ROW);
            plot_heart(base, 600, HEART_ROW);
            clear_16_bitmap(base, 580,HEART_ROW); 
            break;
        case 1:
            plot_heart(base, 610, HEART_ROW);
            clear_16_bitmap(base, 600,HEART_ROW); 
            clear_16_bitmap(base, 580,HEART_ROW); 
            break;
        case 0:
            clear_16_bitmap(base, 610,HEART_ROW); 
            clear_16_bitmap(base, 600,HEART_ROW); 
            clear_16_bitmap(base, 580,HEART_ROW); 
    }
}
